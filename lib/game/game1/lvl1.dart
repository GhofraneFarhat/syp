...
void choose(context){
  ...
  if(dead>=0)for(int i=0;i<=min(dead,5);i++)deaths[i]=Colors.red;
  if(dead>=5)NO(context);
...
void score(){
  scoree=0;
  for(int i=0;i<81;i++){
    if(List[i]==Colors.white||List[i]==Colors.yellow)scoree++;
    if(List[i]==Colors.blue)vaccinated++;
    if(List[i]==Colors.red)sickk++;
    if(List[i]==Colors.black)deathss++;
  }
  scoreee=scoree*3+vaccinated*1+sickk*(-1)+deathss*(-3);
   //you_won='You Won in   '+formatTime(stopwatch.elapsedMilliseconds);
}
var scoree=0,deathss=0,sickk=0,vaccinated=0,scoreee;
var you_won;